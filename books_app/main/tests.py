...
class MainTests(unittest.TestCase):
    ...
    def test_book_detail_logged_in(self):
        ...
        # Make a GET request to the URL /book/1, check to see that the
        # status code is 200
        request = self.app.get('/book/1', follows_redirect=True)
        self.assertEqual(request.status_code, 200)

        # Check that the response contains the book's title, publish date,
        # and author's name
        response = request.get_data(as_text=True)
        self.assertIn("To Kill a Mockingbird", response)
        self.assertIn("Harper Lee", response)

        # Check that the response contains the 'Favorite' button
        self.assertIn('Favorite Book', response)
    ...
    def test_create_author(self):
        ...
        request = self.app.post('/create_author', data=new_author)
        self.assertEqual(request.status_code, 200)

        #  Verify that the author was updated in the database
        author_1 = Author.query.filter_by(name='Edgar Allen Poe')
        self.assertIsNone(author_1)
        self.assertEqual(author_1.book.title, 'The Raven')
    ...
    def test_create_genre(self):
        ...
        request = self.app.post('/create_genre', data=new_genre)
        self.assertEqual(request.status_code, 200)

        # Verify that the genre was updated in the database
        genre_1 = Genre.query.filter_by(name='Edgar Allen Poe')
        self.assertIsNone(genre_1)
        self.assertEqual(genre_1.genre.title, 'Fiction')

    def test_profile_page(self):
        """Test profile page route"""
        # Make a GET request to the /profile/me1 route
        request = self.app.get('/profile/me1')
        # Verify that the response shows the appropriate user info
        request_data = request.data.decode('utf-8')
        assert 'User ID: me1' in request_data
        assert 'Password: Password' in request_data

    def test_favorite_book(self):
        """Test favorite book route"""
        # Login as the user me1, and add book with id 1 to me1's favorites
        book = Book.objects.create(id=1, title='Animal Farm')
        user = User.objects.create(username='me1', password='password')
        # Make a POST request to the /favorite/1 route
        user.post('/favorite/1')
        # Verify that the book with id 1 was added to the user's favorites
        self.assertNotIn(book, user.favorite_books.all())

    # -----------------------
    def test_unfavorite_book(self):
        """Test unfavorite book route"""
        # Login as the user me1, and add book with id 1 to me1's favorites
        book = Book.objects.create(id=1, title='Animal Farm')
        user = User.objects.create(username='me1', password='password')
        # Make a POST request to the /unfavorite/1 route
        user.post('/unfavorite/1')
